...
2. 问安装模式（**默认轻装**）——两模式语义（安装由步骤 3 的脚本机械执行，AI 不手抄文件）：
   - **轻装（默认）**：项目根 `CLAUDE.md` 仅数行——`@<crules 源>/CLAUDE.md` 导入行 + `## 本项目覆写` 空节 + **版本戳** `<!-- crules: v<源版本> @ <日期> -->`。规则正文**单一权威在 crules 源**，源更新后本项目下次会话自动生效（零 cp 零合并）；**导入行指向稳定路径**（crules 仓库路径优先；plugin cache 版本目录有静默漂移风险，脚本装时会自动警告）；**团队场景**：轻装要求 crules 仓库路径可达——只装 plugin 没有仓库者选完整模式；项目特殊约定写覆写节，**覆写优先于导入**
   - **完整**：全量 `CLAUDE.md`（头部带版本戳；团队多机 / 高定制 / 不依赖源路径时选；复制即分叉，更新重跑本命令合并）
3. **跑安装脚本**（确定性操作脚本化，幂等可测；源定位同步骤 0，cache 路径亦可用——安装是一次性动作无漂移问题，lite 导入行指 cache 时脚本自动警告）：
   ```bash
   bash <crules源>/scripts/install.sh <项目根> --mode lite|full [--dry-run] [--force]
   ```
   脚本行为：目标已有**无戳** CLAUDE.md → 中止（那是老项目，走 2b）；有戳 → 重装（默认跳过已存在，`--force` 覆盖）；`--dry-run` 只出安装报告；装完输出写入/跳过清单与下一步。需求方想先看全景报告时先跑 `--dry-run`
4. 引导填 `项目附录.md` 的 **3 个必填**（项目名 / 技术栈 / 构建·分析·测试命令）；顺带**提示一处减压阀**：附录「协作偏好」节可声明**提速档**（扩大极简范围、免方案直接实施——仅需求方可调，新手不知道有这个就白挨重流程）与大需求触发词的**本项目映射**（如后端项目：新接口 / 跨服务 / 数据迁移）
5. 提示：轻装模式依赖 crules 源路径稳定，源移动后须改导入行（巡检：`bash <crules源>/scripts/check-imports.sh <项目根>`）；进阶篇按「启用条件」门控自开；记忆库启用时按 `CLAUDE.md` §七 加会话级声明
