...
1. **说什么做什么**：只描述需求；AI 发现范围外问题会报告，不顺手修
2. **贴报错 = 诊断，不是授权改码**：想让它修，明确说「修复它」
3. **非平凡改动先看方案**：AI 会先给方案等你确认再动码——这是特性不是拖延
4. **提交只认关键词**：说 `commit` 本地提交、`push` 推远端；不说，它不碰 git
5. **完成必带验证输出**：AI 说「完成 / 修复」时必须附命令输出或证据；没有就追问

## 三档规模（按项目选，从 Lite 起步）

| 档 | 装什么 | 适合 |
|---|---|---|
| **Lite** | 根规则（轻装 `@` 导入）+ plugin hooks 硬闸 | 个人项目 / 快速起步——红线与提交纪律即生效 |
| **Team** | Lite + `项目附录.md` 全填 + `agents/` 三角色 + `memory/` 记忆库（制度资产进 git，团队共享） | 团队多机——统一规则 + 审查角色 + 共享记忆 |
| **Advanced** | Team + `进阶/` 全六篇（方案评审闭环 / Agent 编排 / 记忆库体系）+ 记忆库启用声明 | 大项目 / 高风险需求——双 Gate 强化 + 独立评审 + 知识分层 |

三档不必预先选死：`/crules:crules-init` 默认即 Lite 形态，进阶篇自带「启用条件」门控用到才开——扩档 = 复制对应目录 + 填附录，随时可升。
