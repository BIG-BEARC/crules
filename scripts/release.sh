#!/usr/bin/env bash
# crules plugin 发布辅助（v49，审评阶段 2）——把 v31「bump 必 update + cache grep」机械步骤脚本化
# 用法：
...
PY
    ...
    echo "== 下一步（手工）== claude plugin update crules@crules-market && scripts/release.sh verify-cache '<本轮改动特征串>'"
    ;;
esac
